def get_problems
  ...
  input(array, 'eventのuser_idとuserのidで内部結合', 'select * from events inner join users on events.user_id = users.id;')
  input(array, 'eventのuser_idとuserのidで左外部結合', 'select * from events left outer join users on events.user_id = users.id;')
  input(array, 'eventのuser_idとuserのidで右外部結合', 'select * from events right outer join users on events.user_id = users.id;')
  ...
  input(array, 'user_idが1か2のtitleを表示', 'select title from events where user_id in(1,2);')
